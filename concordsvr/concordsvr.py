...
DEVICEID = [ #this is all handled programmatically now. Don't touch!
	"", #panel ID
	"", #zone 1 ID
	"", #zone 2 ID
	"", #zone 3 ID
...
class Concord4ServerConfig():
	...
	self.HUB_CALLBACKURL_BASE = self.read_config_var('main', 'hub_callbackurl_base', '', 'str')
        self.HUB_CALLBACKURL_APP_ID = self.read_config_var('main', 'hub_callbackurl_app_id', '', 'str')
        self.HUB_CALLBACKURL_ACCESS_TOKEN = self.read_config_var('main', 'hub_callbackurl_access_token', '', 'str')
        self.HUB_CALLBACKURL_CONCORD_DEVICE_ID = self.read_config_var('main', 'hub_callbackurl_concord_device_id', '', 'str')
...
class ConcordSvr(object):
    ...
    def startup(self):
        ...
        #st_request.start()
	#hub_request = HubitatUpdate("",self,senddelay=0) #this is to acquire the device id for the panel
        #hub_request.start()
	#time.sleep(5)
	hub_request = HubitatUpdate('/'+config.HUB_CALLBACKURL_CONCORD_DEVICE_ID+'/statusUndefined',self,senddelay=0)
	hub_request.start()
    ...
    def panelMessageHandler(self, msg):
        ...
        if cmd_id == 'PANEL_TYPE':
            ...
            if len(new_zone_state) == 0:
                ...
                #st_request.start()
		hub_request = HubitatUpdate('/'+str(DEVICEID[zone_num])+'/close',self,senddelay=delay)
                hub_request.start()
                ...
                #st_request.start()
		hub_request = HubitatUpdate('/'+str(DEVICEID[zone_num])+'/open',self,senddelay=0)
                hub_request.start()
                ...
                #st_request.start()
		hub_request = HubitatUpdate('/'+str(DEVICEID[zone_num])+'/open',self,senddelay=delay)
                hub_request.start()
            ...
            if int(msg['arming_level_code']) == 1:
                ...
                #st_request = SmartThingsUpdate('armstatus/disarmed',self,senddelay=0)
                #st_request.start()
		hub_request = HubitatUpdate('/'+str(DEVICEID[0])+'/statusdisarmed',self,senddelay=0)
                hub_request.start()
                ...
                #st_request.start()
		hub_request = HubitatUpdate('/'+str(DEVICEID[0])+'/statusarmedstay',self,senddelay=delay)
                hub_request.start()
                ...
                #st_request.start()
		hub_request = HubitatUpdate('/'+str(DEVICEID[0])+'/statusarmedaway',self,senddelay=delay)
                hub_request.start()
...
class ConcordHTTPServer(asyncore.dispatcher,Thread):
	...
	logger('header: '+str(header))
		...
		logger('armed:  '+str(self.getPartitionState(part_key)))
            ...
            elif '/concord/config' in path:
		logger('DEVICEID (Before): '+str(DEVICEID))
		path_split = path.split('/concord/config=')
		DEVICEID = path_split[1].split('&')
		logger('DEVICEID (After): '+str(DEVICEID))
		hub_request = HubitatUpdate('/'+str(DEVICEID[0])+'/statusConfiguredConcord',self,senddelay=0)
                hub_request.start()
            else:
                channel.pushstatus(404, "Not found")
                channel.push("Content-type: text/html\r\n")
                channel.push("\r\n")
		hub_request = HubitatUpdate('/'+str(DEVICEID[0])+'/statusHelpConcord',self,senddelay=0)
                hub_request.start()
...
class HubitatUpdate(Thread):

    def __init__(self,url,concordSvr,method='get',senddelay=0):
        super(HubitatUpdate, self).__init__()
        self.daemon = True
        """Initialize"""
        self.url = url
        self.method = method
    ...
    def run(self):
	global DEVICEID
        hub_URL_prefix = config.HUB_CALLBACKURL_BASE + "/apps/api/" + config.HUB_CALLBACKURL_APP_ID + "/devices" # + config.HUB_CALLBACKURL_CONCORD_DEVICE_ID + "/"
        hub_URL_suffix = "?access_token=" + config.HUB_CALLBACKURL_ACCESS_TOKEN
	headers = {'Authorization': 'Bearer {}'.format(config.HUB_CALLBACKURL_ACCESS_TOKEN)}
        try:
            if 'get' in self.method:
                time.sleep(self.senddelay) #dont update hubitat, wait until panel is disarmed
                r = requests.get(hub_URL_prefix+self.url+hub_URL_suffix,timeout=20,headers=headers)
                logger('Hubitat TX -> '+hub_URL_prefix+self.url+hub_URL_suffix)
		log.debug('r.text: '+str(r.text))
		log.debug('self.url: '+str(self.url))
                if (r.status_code != 200):
                    logger('Hubitat TX Failed: ' + str(r.status_code)+' url:'+self.url)
		if self.url:
			log.debug('Valid self.url detected')
		else:
			logger('Blank self.url detected')
			converted_string = list(eval(r.text))
			x = next(item for item in converted_string if item["name"] == "Concord 4 Panel")
    			DEVICEID[0] = x.get('id')
			logger('DEVICEID[0]: '+str(DEVICEID[0]))
        except Exception as ex:
            tb = traceback.format_exc()
            logger('Hubitat TX Exception: '+ str(ex.message)+'url:'+self.url)
            log.debug('TRACEBACK:'+str(tb))
